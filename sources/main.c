...
static t_info	*tinfo_init(char *argv)
{
	t_info *info;

	if (!(info = (t_info *)ft_memalloc(sizeof(t_info))))
		set_error(MALLOC);
	if (!(info->meta = (t_meta *)ft_memalloc(sizeof(t_meta))))
		set_error(MALLOC);
	info->meta->width = WTH;
	info->meta->heigth = HTH;
	info->meta->go_x = info->meta->width * 0.5;
	info->meta->go_y = info->meta->heigth * 0.5;
	info->meta->amount = blocks_counter(argv);
	info->meta->scale = 2;
